...
if __name__ == '__main__':
    ...
    with warnings.catch_warnings():
        ...
        for tile in strr[1:]:
            ...
            rclone.with_config(cfg).run_cmd(command="copy", extra_args=[f"ceph:zarrs/wsf/landscan2015/population_{tile}", f"/landscan2015/population_{tile}"])
            ...
            pop_path = f"/landscan2015/population_{tile}"
            pop_zarr = zarr.open(pop_path, mode='r')
            bnd_path = f"bnd/{tile}"
            bnd_zarr = zarr.open(bnd_path, mode='r')

            h_valid = elec_zarr['Pixel_H'][np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]
            v_valid = elec_zarr['Pixel_V'][np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]

            h_valid_glob = elec_zarr['Global_Pixel_H'][np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]
            v_valid_glob = elec_zarr['Global_Pixel_V'][np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]

            h_not = [i for i in range(len(elec_zarr['Pixel_H'])) if i not in np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]
            v_not = [i for i in range(len(elec_zarr['Pixel_V'])) if i not in np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]
            h_not_glob = [i for i in range(len(elec_zarr['Global_Pixel_H'])) if i not in np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]
            v_not_glob = [i for i in range(len(elec_zarr['Global_Pixel_V'])) if i not in np.where(elec_zarr['Electrification_Date_Start'] <= np.datetime64("2015-07-01"))[0]]
            ...
            with open(f"{tile_stripped}_val_2015.json", "w") as f:
                json.dump(final, f) 

            rclone.with_config(cfg).run_cmd(command="copy", extra_args=[f"{tile_stripped}_val_2015.json", f"ceph:zarrs/wsf/landscan_elec_val_2015_1lmin_20percent_30valid/"])
